...
var ClientPositions = function(){
    ...
    ClientPositions.prototype.list = function(callbacks){ 
                        ...
                        this.get(null, client_id, function(list){
                            ...
                            var add_item = function(editor, item){
                                var delete_btn = !item.client_id ? '<i class="fa fa-minus w3-red p-1 pointer rounded invisible"></i>' : '<i class="fa fa-minus w3-red p-1 pointer rounded del"></i>';
                                var row = $('<div class="col-12 my-1">'+delete_btn+'<b class="mx-2 border-right border-dark "></b><span class="h6 font-weight-bold link">'+ucwords(item.value, true)+'</span></div>');
                                ...
                                row.find('i.del').data('id', item.id).click(function(){
                                    ...
                                    context.delete(id, client_id, function(success){if(success){ row.remove(); }})
                                });
                            ...
                            var editor = view_modal(content, 'Positions', function(action){
                                    ...
                                    if(action.action == "cancel"){
                                        ...
                                        context.save(null, client_id, new_position, function(resp){
                                            add_item(editor, {"id":resp.success, "value":new_position, "client_id":client_id });
                                            reset_editor(editor);
                                        })
                                    }
                            }, true, 'md', false);
    ...
    ClientPositions.prototype.edit = function(id, position, cb, beforeCb){
                                ...
                                text_editor(position, function(value, editor){
                                    ajax_post("modules/clients/app/clientUsers.php?cmd=save_position", {"id":id, "value": value},
    ...
    ClientPositions.prototype.get =   function(id, client_id, cb){
        ...
        ajax_get_no_loading("modules/clients/app/clientUsers.php?cmd=get_position", {"id": id, "client_id":client_id},
    ...
    ClientPositions.prototype.save =   function(id, client_id, position, cb){
        ...
        ajax_get("modules/clients/app/clientUsers.php?cmd=save_position", {"id": id, "client_id":client_id, "value":position},
                function(resp){ if(!checkError(resp, true)){ execfunc(cb, resp); } }, function(resp){error('Error while retrieving items !'); console.log(resp.responseText); });
    ...
    ClientPositions.prototype.delete =   function(id, client_id, cb){ 
        ...
        delete_elements({'id':id}, "modules/clients/app/clientUsers.php?cmd=delete_position&client_id="+this.client_id, 
