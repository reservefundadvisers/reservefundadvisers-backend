...
{  
    ...
    public function simulation($data){
        ...
        foreach($model_items as $item){
            ...
            for($i=0; $i<$redundancies; $i++){                
                ...
                $actual_cost = check_val($spendings_actuals, "$item_id/$i", $cost);
        ...
        // LTIM Option to carry on
        $ltim_perc = $rule_ltim_perc;         // % to invest in LTIM
        $ltim_reinv = 1;
        ...
        for($i=0; $i<$period; $i++){
            ...
            */
            ...
            if($is_ltim_enabled){
                ...
                // reinvest accumulated LTIM funds
                $ltim_reinv = check_val($erase_deficit, 'ltim_reinv', $ltim_reinv) == 1;

                if(!$ltim_reinv){
                    $ltim_withdrawn += check_val($calculated[$i], 'ltim_acc', 0);
                    $year_calculations['ltim_acc'] = 0; 
                    $year_calculations['ltim_acc_p'] = 0; 
                    $year_calculations['ltim_acc_ne'] = 0; 
                    $starting_amount += $ltim_withdrawn;
            ...
            if($is_ltim_enabled){
                ...
                $year_calculations['ltim_reinv'] = $ltim_reinv ? 1 : 0; 
            ...
            if(!$is_lowering_fees && !$applying_cushion /* && ($auto_monthly_fees_stop_year == -1 || $i < $auto_monthly_fees_stop_year)  */
                ...
                // add extra deficit to year using auto-fee
                if($current_auto_fee == $i && $auto_monthly_fees_inc[$i] < $rule_mf_perc){
    ...
    public function calculateYearData(  $starting_amount,                                         
            ...
            $tmp_year_calculations = [  
                                        ...
                                        "ltim_perc"=>$impl_ltim ? $ltim_perc : 0
    ...
    private function updateMonthlyFeeInc(   $year, $deficit, 
            ...
            */
            ...
            // log_info("deficit: $deficit, year: ".($year+2018).", start_year: ".($start_year+2018).", total_years: $total_years, incBy: $inc");
    ...
    private function get_rules($model_id){
        ...
        return !empty($rules) ? $rules : [  "ltim_enabled"=>0, "ltim_perc"=>1,
