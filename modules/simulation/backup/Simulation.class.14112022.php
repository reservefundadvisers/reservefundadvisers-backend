...
{  
    ...
    public function simulation($data){
        ...
        $prev_monthly_fees = $monthly_fees;
        ...
        // log_info(implode(", ", $auto_monthly_fees));
        /* ************** */
        ...
        for($i=0; $i<$period; $i++){
            ...
            if($is_ltim_enabled ){
                ...
                $ltim_withdrawn = $auto_monthly_fees_ltim_wth[$i];
            ...
            $this->calculateYearData(   $starting_amount,                                         
                                        ...
                                        ($rule_mf_auto ? 0 : $loan_amount),
                                        $loan_payment,
                                        ($rule_mf_auto ? 0 : $assessment),
                                        $spending, 
                                        $erase_deficit,
            ...
            if($is_ltim_enabled){
                if(!isset($erase_deficit['ltim_wth']) && $rule_ltim_cover == 1 && $year_calculations['fa'] < 0){
                    ...
                    $ltim_available = check_val($year_calculations, 'ltim_acc', 0) * 0.8; // only withdraw 80%
            ...
            if(!$is_lowering_fees && !$applying_cushion /* && ($auto_monthly_fees_stop_year == -1 || $i < $auto_monthly_fees_stop_year)  */
                ...
                $this->updateMonthlyFeeInc($i, ($year_calculations['fa']/(12*$housing)), $auto_monthly_fees, $auto_monthly_fees_inc, $rule_mf_perc, $monthly_fees, $rule_cushion_fund, $auto_monthly_fees_stop_year);
            ...
            // check last MF increase
            if($prev_monthly_fees < $year_calculations['mf']){
                $last_deficit = $i;
            }
            $prev_monthly_fees = $year_calculations['mf'];
    ...
    private function initOriginalCalculation(   &$calculated, &$default_calculated, &$deficit_years,
        ...
        $default_calculated;
        $year_calculations = array_fill($start_year, $period, []);
    ...
    private function updateMonthlyFeeInc($year, $deficit, &$auto_monthly_fees, &$auto_monthly_fees_inc, $max_inc, $default_monthly_fee, $cushion_fund = 0, $stop_at_year = -1){
            ...
            $inc_p = $this->get_closest_inc_year($year, 0, $auto_monthly_fees, $auto_monthly_fees_inc, $max_inc, '', $deficit);
            $start_year = $inc_p['start_year'];
            $total_years = $inc_p['total_years'];
            $inc = $inc_p['inc'];
            ...
            $period = $stop_at_year >= 0 ? $stop_at_year : count($auto_monthly_fees);
