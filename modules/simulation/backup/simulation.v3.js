...
var Simulation = function(){
    ...
    Simulation.prototype.eraseDeficit = function(at, callback){
        ...
        // set title 
        deficit_box.find('.deficit-title').html(at == 0 ? 'Current Year' : 'Year '+ (parseInt(at)+1));

        
        // remaining amount list
        var fa_row = deficit_box.find('.simulation-fa');

        deficit_box.find('.simulation-fa-toggle').unbind().click(function(){ $('.simulation-fa-parent').toggleClass('d-none'); });
        
        // toggle fa list when lower than lg
        var window_prev_width = 0;
        var toggle_fa_position = function(to){            

            var is_not_lg = ($(window).width() < 992);
            if(window_prev_width != $(window).width() && is_not_lg )
                $('.simulation-fa-parent').addClass('d-none');
                
            $('.simulation-fa-parent').toggleClass('position-absolute shadow', is_not_lg);

            window_prev_width = $(window).width();
        }
        toggle_fa_position();
        ...
        var print_fa = function(){
                                ...
                                for(const c of context.simulation_data.calculated){
                                    ...
                                    fa_row.append(' <div class="col-4 p-1  border-bottom border-dark '+(c.year == at ? 'bg-primary' : bg_color + ' year link underline')+'" data-year="'+c.year+'"> \
                                                            '+(c.year == 0 ? 'Current' : 'Year '+(parseInt(c.year)+1) ) + /* ' <span class="w3-green">'+formatMoney(c.mf, 0)+'</span>' + has_data + */ '   \
                                                    </div>  \
                                                    <div class="col-4 p-1 border-bottom border-left border-dark text-right '+(c.fa >= 0 ? 'w3-text-black' : (c.ltim_acc > 0 && c.ltim_acc >= ( -1 * c.fa ) ? 'w3-text-blue' : 'w3-text-red'))+'">    \
                                                        '+formatMoney(c.fa, 0)+'    \
                                                    </div>  \
                                                    <div class="col-4 p-1 border-bottom border-dark text-right '+(c.fa_o >= 0 ? 'w3-green' : 'w3-red')+'">    \
                                ...
                                // hide Years Table if lower than lg
                                $('.simulation-fa-parent').addClass('d-none');
        ...
        var disable_nav = function(tab){
            ...
            var navs = deficit_box.find('.simulation-tab-nav'),
                fa_overlay = deficit_box.find('.simulation-fa-overlay');
            

            if(!tab){navs.removeClass('invisible'); fa_overlay.addClass('d-none'); return;}
        ...
        // select specific nav
        var select_nav = function(tab){

            var navs = deficit_box.find('.simulation-tab-nav');
            

            if(!tab){navs.first().click(); return;}

            navs.filter('[id^="'+tab+'"]').first().click();
        ...
        var eraseDeficit_init = function(){
            ...
            var monthly_fees = !objEmpty(deficit_data) && deficit_data.monthly_fees ? parseFloat(deficit_data.monthly_fees) : original_data.mf;
            ...
            deficit_box.find('.simulation-compare').unbind().click(function(){
                    ...
                    context.compare(context.model_id, function(resp){
                            ...
                            var managed = resp.managed.calculated || {};
                            var ltim = resp.ltim.calculated || {};

                            var chart_series = { "collections_m":[], "deficit_m":[], "collections_l":[], "deficit_l":[] };
                            var chart_xaxis = [];
                            ...
                            var chart_initial_data = [	
                                                ...
                                                { name: 'Collections LTIM', type: 'bar', barMaxWidth:15,  /*stack: 'Collections',*/
                                                            itemStyle: {color: '#ffc107', decal: { color: "#745806", dashArrayX: [1, 0], dashArrayY: [4, 3], rotation: -Math.PI / 4 }}, 
                                                            emphasis: { focus: 'series' }, data: chart_series.collections_l },
                                                
                                                { name: 'Deficit', type: 'bar', barMaxWidth:15, /*stack: 'Collections',*/
                                                            itemStyle: {color: '#ff0000'}, emphasis: { focus: 'series' }, data: chart_series.deficit_m },

                                                { name: 'Deficit LTIM', type: 'bar', barMaxWidth:15,  /*stack: 'Collections',*/
                                                            itemStyle: {color: '#aa0000', decal: { color: "#ff0000", dashArrayX: [1, 0], dashArrayY: [4, 3], rotation: -Math.PI / 4 }}, 
            ...
            // Simultaion General Rule
            var update_general_rules = function(key, value, callback){ if(typeof key == 'object'){ general_rules = key; } else { general_rules[key] = value; } context.updateRules(context.model_id, general_rules, function(){ execfunc(cb); execfunc(callback)});}
                ...
                if(item.is(':checkbox')){
                    item.prop('checked', general_rules[key] && general_rules[key] == 1).change(function(){
                        var item = $(this)
                            key = item.data('key'),
                    ...
                    item.val(is_perc && general_rules[key] ? parseFloat(general_rules[key] * 100): 0).change(function(){
                        var item = $(this)
                            key = item.data('key'),
                            is_perc = item.hasClass('percent'), 
                            value = item.val();
            ...
            var is_ltim_mode = (general_rules['ltim_enabled'] && general_rules['ltim_enabled'] == 1);
                                ...
                                .change(function(){
                                            ...
                                            update_general_rules("ltim_enabled", enabled, 
                                                                    function(){ 
                                                                            ...
                                                                            // select_nav(enabled ? 'ltim' : null);
                                                                    });

                                            /*
                                            if(!has_deficits()){
                                                update_general_rules({"ltim_perc":1, "ltim_cover":1});
                                            }else{

                                                confirm('Boosting With LTIM will reset all your <b>Managed Data</b>, Are you sure you want to <b>Boost</b> ?', '', null, 
                                                    function(action){
                                                        if(action != 'ok')return;

                                                        context.reset(context.model_id, function(){
                                                            update_general_rules({"ltim_perc":1, "ltim_cover":1});
                                                        }, 0, true);
            ...
            // Change Reset Current title
            $('.reset-simulation[data-action="current"]').html("Reset "+(is_ltim_mode ? 'LTIM' : 'Regular')+" Mode Data To Default")
                                                         .toggleClass('w3-purple', is_ltim_mode)
                                                         .toggleClass('w3-red', !is_ltim_mode);
            ...
            ltim_ne_label.html(formatMoney(original_data.ltim_ne, 0));
                ...
                ltim_perc_apply.unbind().click(function(){           
                                                ...
                                                if(ltim_perc > value && has_deficits(at, 'ltim_perc')){
                                                    confirm('Lowering the Amount Allocated to the LTIM Strategy will <b>Reset All LTIM withdraws Starting Year <span class="w3-text-red"> '+(at + 2)+'</span></b>, Are you sure you want to apply ?', 'Warning', null, 
                                                    function(action){ 
                                                                                    if(action != 'ok')return;
            ...
            var ltim_acc_label = deficit_box.find('.ltim-acc'),    
                ...
                ltim_withdraw_apply.unbind().click(function(){           
                                                ...
                                                if(Math.ceil(ltim_withdrawed) < value && has_deficits(at, 'ltim_perc')){
                                                    confirm('Raising the Amount to Withdraw from the LTIM Funds will <b>Reset All LTIM withdraws Starting Year <span class="w3-text-red"> '+(at + 2)+'</span></b>, Are you sure you want to apply ?', 'Warning', null, 
                                                    function(action){ 
                                                                        if(action != 'ok')return;
            ...
            var monthly_fees_slider = deficit_box.find('.monthly-fees-slider'),
                ...
                monthly_fee_increase_update_value = function(value){
                                                            ...
                                                            if(value != undefined){
                                                                update_deficit_data('monthly_fees', Math.ceil(value), Math.ceil(value) == default_monthly_fees);
            ...
            var assessment_update_value = function(value = 0){
                                                    ...
                                                    assessment_unit_label.html(formatMoney(value/units, 2));
                                            }
            ...
            var spendings_check = function(withAlert = false){
                        ...
                        for(var i=0; i<spendings_edit.length; i++){
                            ...
                            if(!s.org){
                                ...
                                var split_of = spendings_edit.find((obj)=>{ return obj.id == s.splitId }); // item from which it's splitted
            ...
            var spendings_fill = function(){
                ...
                var spendings_tmpl = '  <div class="col-12 m-0 p-0 pt-3">    \
                                        ...
                                        <div class="col-3 p-2 pt-1 border-bottom border-dark"><input type="number" min="1" step="1" class="app-input year" data-field="Year" value=""  data-index="%INDEX%" /></div>   \
                                        <div class="col-4 p-2 pt-1 border-bottom border-dark text-center"><input type="number" min="1" step="1" data-field="Expected Life"  class="app-input redundancy" data-index="%INDEX%" readonly /></div>   \
                                        <div class="col-4 p-2 pt-1 border-bottom border-dark">  \
                                            <input type="number" min="1" step="1" class="app-input cost org-cost" data-field="Cost" value=""  data-index="%INDEX%" data-split="" readonly /> \
                                        </div>    \
                                        <div class="col-1 p-0 text-left pt-1 border-bottom border-dark">    \
                ...
                if(spendings.length == 0){
                    spendings_table.append('<div class="col-12 text-center">There is no <b>Expenses</b> in this year !</div>');
                }else{
                        ...
                        for(var i=0; i<spendings_edit.length; i++){
                            ...
                            element.find('.name').html(
                                                            !s.id && !s.org ? '<i class="ml-5 fa fa-level-up-alt fa-rotate-90"></i>' 
                                                                            : (ucwords(s.name, true)  + ('<b class="badge ml-1 w3-pink">'+(s.redundancy_at+1)+'</b>') + (s.split_of ? '<b class="badge ml-1 w3-deep-purple">Split</b>' : '') ) );
                            
                            element.find('input.year').val(parseInt(s.year) + 1)
                                    .on('input', function(){ 
            ...
            deficit_box.find('.spendings-apply').unbind().click(function(){ 
                    ...
                    if((spendings_final_edit.length > 0 || spendings_unsplit.length > 0)){
                        ...
                        if(has_deficits(first_year)){
                            ...
                            confirm('Changing Spendings will <b>Reset All Simulation Managed Data Starting Year <span class="w3-text-red"> '+(first_year + 1)+'</span></b>, Are you sure you want to apply ?', 'Warning', null, 
                                        function(action){ 
                                                                        if(action != 'ok')return;
            ...
            var inv_strategy_check = function(){
                
            }

            var tmp_inv = inv_strategy; // && !objEmpty(inv_strategy) ? inv_strategy : context.simulation_data.model.inv_strategy;
            for(const i in inv_strategy){
                inv_strategy_inputs.filter('[data-index="'+i+'"]').filter('[data-type="rate"]').val(tmp_inv[i].rate || 0).data('old', tmp_inv[i].rate || 0);
                inv_strategy_inputs.filter('[data-index="'+i+'"]').filter('[data-type="perc"]').val(tmp_inv[i].perc || 0).data('old', tmp_inv[i].perc || 0);
            }
                                    ...
                                    var item = $(this),
                                        ...
                                        inv_strategy_inputs.each(function(){
                                            var tmpItem = $(this), 
                                                ...
                                                newVal = parseFloat(tmpItem.val() || 0),
            ...
            var apply_changes = function(){
                    ...
                    if(deficit_data['inv_strategy'] && Array.isArray(deficit_data['inv_strategy'])){
    ...
    Simulation.prototype.init = function(model_id, callbacks){ 
            ...
            this.simulation(model_id, function(data){
                ...
                var formatter = function(row){
                    ...
                    for(var c of cells){
                        ...
                        if(k == "year"){
                            cell.html((data.year == 0) ? 'Current' : i + 1);
                            continue;
                        }
                ...
                var print_table = function(){
                    ...
                    var chart_series = {"remaining_amount":[], "compound":[], "total_expenses":[], "loan":[], "loan_payments":[], "collections":[], "ltim":[], "ltim_i":[], "deficits":[]};
                    var chart_xaxis = [];
                    //chart_element.html('');
                    ...
                    for(var i=0; i<calculated.length; i++){
                        ...
                        chart_series.compound.push((c.cp).toFixed(2));
                        ...
                        chart_xaxis.push(parseInt(c.year) + 1);
                    ...
                    var chart_initial_data = [	
                                                { name: 'Compound', type: 'line', barMaxWidth:15, /* stack: 'amount', */
                                                ...
                                                { name: 'Loan Payments', type: 'bar', barMaxWidth:15,
                                                            itemStyle: {color: '#d387a3'}, emphasis: { focus: 'series' }, data: chart_series.loan_payments },
                    ...
                    if(context.simulation_data.model.simulation_rules.ltim_enabled){
                        chart_initial_data.push({ name: 'Projected LTIM Funds', type: 'bar', barMaxWidth:15, stack: 'inv', 
                                                    itemStyle: {color: '#4CAF50'}, emphasis: { focus: 'series' }, data: chart_series.ltim });
                    }
            ...
            }, function(){ l.remove(); execfunc(callbacks, {'data': null}); }, true);
    };
    ...
    // Simulation.prototype.reset =   function(model_id, cb, which = ''){
        
    //     if(!model_id){ return false; }

    //     ajax_post("modules/simulation/app/simulation.php?cmd=reset", {"model_id":model_id, "which":which},
    ...
    Simulation.prototype.tour_guide = { "main": [   {text: 'Click here to <b>Reset All Simulation Data</b> including the <b>Change Priority</b> splits !', audio: 'simulation/help/audio/CM - 1.m4a'},
                                                    'Click here to <b>Reset Simulation Data Only For the Current Mode</b> including the <b>Change Priority</b> splits !',
                                                    'Click here to <b>View The Graph</b> illustrating all calculated data ',
                                                    'Click here to <b>View The Table</b> containing all calculated data ',
                                                    'Click here to change the <b>Simulation Settings</b> that will be applied to the simulation',
                                                    'Click here to switch between <b>Regular Mode</b> and <b>LTIM mode</b>',
                                                    'Click here to view the <b>Compare Graph</b> to compare data between Regular and LTIM mode',
                                                    {text: 'Click here when available to <b>Reset the currently selected year\'s Data</b>, this will only affect the mode you are currently working on', audio: 'simulation/help/audio/CM - 2.m4a'},
                                                    'Click here on any <b>Year</b> to view year\'s data and manage it',
                                                    'Click here to <b>Change Priorities</b> of the expenses of the Year you\'re currently working on',
                                                    'Click here to <b>Change Your Investment Strategy</b> of the Year you\'re currently working on',
                                                    'Click here when <b>LTIM Mode</b> is activated to <b>Change LTIM Strategy</b> of the Year you\'re currently working on',
                                                    'Click here to <b>Change Projected monthly increase</b> of the Year you\'re currently working on',
                                                    'Click here to <b>Change Loan Options</b> of the Year you\'re currently working on',
                                                    'Click here to <b>Change Immediate Assessment</b> of the Year you\'re currently working on',
                                                    'When visible, this will indicate that you\'re workin in <b>LTIM Mode</b>, otherwise, you\'re in <b>Regular Mode</b>',
                                                    'This contains the tabs to manage different options',
                                                    'This will show you the <b>Year\'s Summary Data</b>',
                                                    'This will show you the <b>Year\'s Summary Data</b>' ] };
