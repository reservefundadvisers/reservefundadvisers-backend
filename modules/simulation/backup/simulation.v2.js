...
var Simulation = function(){
    ...
    Simulation.prototype.eraseDeficit = function(at, callback){
        ...
        var print_fa = function(){
                                ...
                                for(const c of context.simulation_data.calculated){
                                    fa_row.append('<div class="col-5 p-2  border-bottom border-dark '+(c.year == at ? 'bg-primary' : 'year link underline')+'" data-year="'+c.year+'"> Year '+(parseInt(c.year)+1)+'</div><div class="col-7 p-2 border-bottom border-dark text-right '+(c.fa >= 0 ? 'w3-green' : 'w3-red')+'">'+formatMoney(c.fa, 0)+'<\div>');
                                }
        ...
        //var deficit_modal = view_modal(deficit_box, '', 
        //                                function(event){ if(event.action == 'cancel'){ execfunc(callback); }; }, false, 'xl', false);
        //deficit_modal.find('.modal-dialog').css('max-width', '95%');
        ...
        var eraseDeficit_init = function(){
            ...
            var deficit_erase = cloneObj(original_data.deficit);
            // if(deficit_erase.fa !=  undefined)amount = parseInt(deficit_erase.fa);
            // if(deficit_erase.ta !=  undefined)total_amount = parseInt(deficit_erase.ta);
            ...
            var init_deficit_data = function(clear = false){ 
                                        ...
                                        var ltim = !objEmpty(deficit_erase) && deficit_erase.ltim && !clear ? parseInt(deficit_erase.ltim) : 0;
                                        ...
                                        if(first_run){original_deficit_data = deficit_data; first_run=false;}
                                    };
            ...
            var calculate_remaining_deficit = function(type = '', returnAsIs = false){
                    
                var remaining = 0;
                    
                    var yc = Math.ceil(12 * units * deficit_data["monthly_fees"]);
                    
                    var diff_inflation = (yc - original_yearlyCollections) * inflation_rate;

                    var new_deficit_amount = deficit_amount + diff_inflation  + original_yearlyCollections + original_net_earning_amount - net_earning_amount;

                    // console.log(new_deficit_amount, deficit_amount, diff_inflation, original_yearlyCollections, original_net_earning_amount, net_earning_amount );

                        //default_monthly_fees = original_yearlyCollections * inflation_rate;

                    if(type == 'mf'){
                        remaining = new_deficit_amount;
                    }else if(type == 'loan'){
                        remaining = deficit_amount;//(yearlyCollections * deficit_data["monthly_fees_inc"]);
                    }else if(type == 'assessment'){
                        remaining = new_deficit_amount - yc - deficit_data['loan_amount'];
                    }else {
                        var loan_p = deficit_data["loan_amount"] ; //+ yearly_intrest();
                        //var yearly_pi = calculate_loan_intrests(deficit_data["loan_amount"], false) / deficit_data["loan_years"];

                        remaining = new_deficit_amount - yc - loan_p /* + yearly_pi */ - deficit_data["assessment"];
                        // console.log(remaining, new_deficit_amount, yc, loan_p, deficit_data["assessment"] );
                        
                    } 

                    remaining = Math.ceil(remaining);
                    
                    if(returnAsIs)return parseInt(remaining);
            ...
            var calculate_remaining_surplus = function(){
                ...
                var deficit_surplus = calculate_remaining_deficit('', true);            
                deficit_surplus = original_surplus_amount + ( deficit_surplus > 0 ? 0 : Math.abs(deficit_surplus) );
            ...
            var ltim_input = deficit_box.find('.ltim-input');
                ltim_label = deficit_box.find('.ltim-amount');
                ltim_input.val(deficit_data['ltim'] || 0);
                ltim_input.on('input', function(){
                                        var value = this.value,
                                            surplus = calculate_remaining_surplus();

                                        if(value < 0)this.value = 0;
                                        else if(surplus > 0 && value > surplus)this.value = surplus;

                                        deficit_data['ltim'] = this.value;
            ...
            var update_remaining_deficit = function(){
                                                ...
                                                var remaining_deficit_amount = original_deficit_amount; // calculate_remaining_deficit();
                                                    surplus_amount = original_surplus_amount; // calculate_remaining_surplus() - deficit_data['ltim'];
                                                ...
                                                // //yearly_pi_label.html(formatMoney(yearly_pi, 0)) ;
                                                ...
                                                //yearly_pi_label.html(formatMoney(yearly_pi, 0)) ;
                                                loss_purchase_power_label.html(formatMoney(original_data.lp, 0)) ;

                                                ltim_label.html(formatMoney(deficit_data['ltim'], 0)) ;
            ...
            var monthly_fees_slider = deficit_box.find('.monthly-fees-slider'),
                ...
                default_monthly_fees = original_data.mf_o;
                ...
                monthly_fee_increase_update_value = function(value){
                                                            ...
                                                            if(value != undefined)deficit_data['monthly_fees'] = Math.ceil(value);
                                                            ...
                                                            */
                                                            ...
                                                            monthly_fees_label.val(Math.ceil(value));                                                
                            ...
                            confirm('Changing Spendings will <b>Reset All Simulation Managed Data Starting Year <span class="w3-text-red"> '+(first_year + 1)+'</span></b>, Are you sure you want to apply ?', 'Warning', null, 
                                        function(action){ 
                                                                        ...
                                                                        utils.deficit(  context.model_id, 
                                                                                        {"year": at, "splits":spendings_final_edit, "unsplit":spendings_unsplit}, 
                                                                                        function(resp){
                                                                                            //deficit_modal.close();
                                                                                            execfunc(cb);
                                                                                        });
            ...
            var inv_strategy_update = function(initOriginal = false){
                ...
                for(const st of deficit_data['inv_strategy']){
                    avr += parseFloat(st.rate || 0 ) * parseFloat(st.perc || 0) / 100;                
                }
                ...
                new_total_amount = (total_amount - original_yearlyCollections - deficit_data['ltim']) + (deficit_data['monthly_fees'] * 12 * units);
                
                net_earning_amount = avr * new_total_amount;
    ...
    Simulation.prototype.init = function(model_id, callbacks){ 
            ...
            var chart_series = {"remaining_amount":[], "compound":[], "total_expenses":[], "loan":[], "loan_payments":[], "collections":[], "ltim":[], "ltim_i":[]};
            var chart_xaxis = [];
            //chart_element.html('');
            ...
            this.simulation(model_id, function(data){
                ...
                var view_spendings2 = function(index, cb){
                    ...
                    var content = $('<div class="w-100 mt-1 mb-4 p-3" style="height:550px">  \
                                        ...
                                        <div class="w-100 mt-2 mb-3 text-right"> \
                                            <label class="btn btn-sm btn-success deficit"><i class="fa fa-check mr-2"></i> Erase Deficit</label> \
                                        </div> \
                                    </div>');
                    ...
                    var spendings_fill = function(){
                        ...
                        for(var i=0; i<spendings_edit.length; i++){
                            ...
                            element.find('.year').val(parseInt(s.year) + 1)
                                    .on('input', function(){ 
                                        var index = $(this).data('index');
                                        var year = parseInt(this.value || 1) - 1;
                                        var tmp = spendings_edit[index];
                ...
                for(var i=0; i<calculated.length; i++){
                    ...
                    chart_series.ltim.push((parseInt(c.ltim) /* + parseInt(c.ltim_i)*/).toFixed(2));
                ...
                var formatter = function(row){
                    ...
                    for(var c of cells){
                        ...
                        if(k == "loan_pi" ){ if( value > 0) { cell.addClass('w3-purple'); } else{ formattedValue = ""; } }
                                ...
                                /*
                                formattedValue = $('<div class="text-left"><span class="w3-'+(objEmpty(data.deficit) ? 'pink' : 'deep-orange')+' w3-hover-deep-purple px-1 rounded manage"><i class="fa fa-cog ml-1"></i> Manage</span></div>');

                                if(!objEmpty(data.deficit))
                                        formattedValue.append('<span class="w3-red w3-hover-deep-orange px-2 ml-2 rounded reset"><i class="fa fa-sync"></i></span>');

                                cell.addClass('pt-1');
                                formattedValue.find('.manage').addClass('erase-deficit')
                                    .data('at', i)
                                        .click(function(){
                                            ...
                                            if(false ){
                                                confirm('Editing will <b>Reset Managed Data Starting Year<span class="w3-text-red"> '+(at+1)+'</span></b>. <br> would you like to continue ?', 'Warning', null, 
                                                        function(action){ 
                                                                if(action != 'ok')return;
                ...
                var chart_initial_data = [	
                                            ...
                                            { name: 'Total Expenses', type: 'line', barMaxWidth:15, /* stack: 'amount', */
                                                        itemStyle: {color: '#550000'}, emphasis: { focus: 'series' }, data: chart_series.total_expenses },
                                            ...
                                            { name: 'LTIM P', type: 'bar', barMaxWidth:15, stack: 'inv', 
