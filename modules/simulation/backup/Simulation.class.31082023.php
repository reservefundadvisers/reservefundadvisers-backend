...
{  
    ...
    private $inv_strategies = [ 'cd'=>'Certificat Of Deposit (CD)', 
                                'cmp'=>'Compound Intrests', 
                                'smp'=>'Simple Intrests', 
                                'ffbp'=>'55 Basis Point'];

                                
    private $inv_strategy_hold = ['cd', 'cmp'];

    private $default_invest_strategy = ['type'=>'ffbp', 'dur'=>3, 'rate'=>0.55, 'year'=>0, 'terms'=>1, 'hold'=>0, 'parent'=>-1, 'index'=>-1];// $model['inv_strategy']; //0.0055;
    ...
    /*
        ...
        calculated "is" object (used to display strategy details)
            ...
            "parent" => >= 0                // parent year of strategy
    ...
    */
    ...
    public function simulation($data){
        ...
        foreach($deficits as $deficit){
            ...
            if(is_valid($deficit_array[$year], "monthly_fees_auto") == 1){
                ...
                if(is_array($deficit_array[$year]['inv_strategy'])){
                    ...
                    // spread strategies that doesn't hold money across
                    // and keep the one that does in their place
                    // reverse order to put current year on top
                    ...
                    for($n=count($tmp_inv_strategies)-1; $n>=0; $n--){
                        ...
                        $strategy['type_name'] = check_val($this->inv_strategies, check_val($strategy, 'type'), 'Investment Startegy');

                        // is type is holdable and terms are > 1
                        $hold = array_has($this->inv_strategy_hold, check_val($strategy, 'type'));
                        ...
                        if(!$hold){
                            for($i=0; $i<$dur; $i++){
                                ...
                                if($year + $i > $period)break;

                                // put current year to top
                                array_unshift($invest_strategies[$year + $i], ['dur'=>1, 'year'=> $i, 'terms'=>$dur, 'hold'=>0, 'parent'=>$year, 'index'=>$n] + $strategy);  // change dur to 1 to pay intrests yearly                                
                            }
                        
                        // holding funds
                        }else{
                            array_unshift($invest_strategies[$year], ['terms'=>$dur, 'hold'=>1, 'parent'=>$year, 'index'=>$n] + $strategy);
                        }
                    }
        ...
        $this->initOriginalCalculation( $calculated, $default_calculated, $deficit_years, 
                                        $starting_amount_o, $monthly_fees, $housing, [], $propagated_inv_strategy, $inflation_rate, $spendings, $period);
        ...
        for($i=0; $i<$period; $i++){
            ...
            // if($rule_inv_keep)$default_invest_strategy = $invest_strategy;
            ...
            */
            ...
            $this->calculateYearData(   $starting_amount,                                         
                                        $auto_monthly_fees[$i], 
                                        $housing,
                                        $invest_strategy, $propagated_inv_strategy,
            ...
            if(!$is_lowering_fees && !$applying_cushion /* && ($auto_monthly_fees_stop_year == -1 || $i < $auto_monthly_fees_stop_year)  */
                ...
                if($current_auto_fee == $i && $auto_monthly_fees_inc[$i] < $rule_mf_perc && $auto_fee_remaining_loop_count < $auto_fee_remaining_loop_max){
                    ...
                    */
                    ...
                    $propagated_inv_strategy = array_fill(0, $period, []);
            ...
            if($i == ($period - 1)){
                ...
                if($last_deficit >= 0){
                    if(!$is_lowering_fees){
                        ...
                        $this->initOriginalCalculation( $calculated, $default_calculated, $deficit_years,
                                                    $starting_amount_o, $monthly_fees, $housing, $default_invest_strategy, $period, $last_deficit+1);
    ...
    private function initOriginalCalculation(   &$calculated, &$default_calculated, &$deficit_years,
                                                $starting_amount_o, $monthly_fees, $housing, $invest_strategies, $propagated_inv_strategy, 
        ...
        for($i=$start_year; $i<$period; $i++){
            ...
            $this->calculateYearData(   $starting_amount_o,                                         
                                        ...
                                        $invest_strategies, $propagated_inv_strategy,
                                        // $year_calculations["lp"],
                                        $inflation_rate,
                                        0,
                                        0,
    ...
    public function calculateYearData(  $starting_amount,                                         
            ...
            if(!empty($propagated_inv_strategy[$year])){
                foreach($propagated_inv_strategy[$year] as $s){
                    if(check_val($s, 'wthd', 0) == 1){
                        $withdrawn_total += check_val($s, 'total', 0);
                        $withdrawn_net_earnings += check_val($s, 'earned', 0);
                    }
                }
            }
                
            
            // addwithdrawn investments to starting amount
            $starting_amount += $withdrawn_total;
            ...
            $inv_principal = (($total_amount < 0 ? 0 : $total_amount) ) - $ltim_p;
            $invest_rate = 0;
            $net_earning = 0;
            $this->calculateClientInvStrategy($invest_strategy, $inv_principal, $ret_strategies, $amount_ot_deduce, $early_penalty);

            // deduce held principal for investments
            $total_amount -= $amount_ot_deduce;
            ...
            foreach($ret_strategies as $ret_strategy){
                ...
                if(isset($ret_strategy['yearly']) && !empty($ret_strategy['yearly'])){
                    ...
                    foreach($ret_strategy['yearly'] as $y => $yearly_details){
                        ...
                        array_push($propagated_inv_strategy[$inv_y], $yearly_details + ['total_pi'=>$ret_strategy_p, 'total_ne'=>$ret_strategy_ne] );

                        if(++$inv_y >= count($propagated_inv_strategy))break;
                    }
            ...
            // log_info("$year -> ad= $amount_ot_deduce, ne = $net_earning, prev_ne = $withdrawn_net_earnings");


            // compound starting amount from investment
            $compound = ceil( ($total_amount < 0 ? 0 : $total_amount) + $net_earning + $withdrawn_net_earnings + $loan  /* + $assessment */ );
    ...
    private function calculateClientInvStrategy($strategies, $inv_principal, &$ret_strategies, &$amount_ot_deduce = 0, &$early_penalty = 0){
        $net_earning = 0;
        ...
        // if single strategy 
        if(is_assoc($strategies))$strategies = [$strategies];
        ...
        $cb = function($s, &$pu, &$ad, &$pn) use ($inv_principal){
            ...
            $type_name = check_val($s, 'type_name', check_val($this->inv_strategies, $type, 'Investment Startegy'));
            
            // does strategy hold funds
            $hold = check_val($s, 'hold', 0) == 1;

            // inv perc and rate
            $rate = check_val($s, 'rate', 0) / 100; if($rate < 0){$rate = 0;}
            $inv_amount = intval(check_val($s, 'amount', 0)); if($inv_amount < 1){$inv_amount = 0;}
            $perc = $inv_amount >= 1 ? ($inv_amount / $inv_principal) : check_val($s, 'perc', 0) / 100; if($perc > 1){$perc = 1;}else if($perc < 0){$perc = 0;}
            
            // duration to run the calculations for
            $dur  = $hold ? check_val($s, 'dur', 1) : 1;  if($dur < 1){$dur = 1;} // how long to run the calculations
            ...
            $extra = [];
            if(isset($s['parent']))$extra['parent'] = $s['parent'];  // parent year
            if(isset($s['index']))$extra['index'] = $s['index'];  // parent inv_stratgery index
            ...
            if($hold){
                ...
                if($perc > 0 && $amount > 0){
                    ...
                    // early withdrawl
                    $y_wth = check_val($s, 'y_wth', $dur);  if($y_wth < 1){$y_wth = 1;}else if($y_wth > $dur){$y_wth = $dur;}
                    ...
                    if($y_wth < $dur){
                        ...
                        $pen_days = check_val($s, 'pd', 1); if($pen_days < 1)$pen_days = 1;
                        $pen_min = check_val($s, 'pm', 0); if($pen_min < 0)$pen_min = 0;

                        $withrawn = round($amount * pow(1 + ($rate/$compounding_frequency), $y_wth * $compounding_frequency));

                        $pen = ($rate / 365) * $pen_days * $withrawn;  // penalty
                        if($pen_min > 0 && $pen < $pen_min)$pen = $pen_min;
                    ...
                    for($i=1; $i<=$inv_dur; $i++){ 
                        ...
                        $new_principal = round($amount * pow(1 + ($rate/$compounding_frequency), ($i-1) * $compounding_frequency)); 

                        // calculate year's P+I
                        $final = round($amount * pow(1 + ($rate/$compounding_frequency), $i * $compounding_frequency)); 
    
                        // add to yearly details
                        array_push($yearly,  [  'type' => $type, 'type_name' => $type_name,
                                                'year' => ($i-1), 'terms'=>$terms, 'hold'=>1,
                                                'rate' => $rate, 'perc'=>$perc, 'no_fund'=>0,
                                                'amount'=>$amount, 'principal'=> $new_principal, 
                                                'total'=>$final, 'earned' => ($final - $amount), 
                                                'wthd' => $i == $inv_dur ? 1 : 0, 'note'=>$note] + $extra); 
                    }
                    ...
                    // add amount to amount_to_deduce when the investment strategy hold the amount
                    $ad += $amount;
                    ...
                    array_push($yearly,  [  'type' => $type, 'type_name' => $type_name,
                                            'year'=>0, 'terms'=>$terms, 'hold'=>1,
                                            'rate' => $rate,'perc'=>0, 'no_fund'=>0,
                                            'amount'=> 0, 'principal'=>0, 
                                            'total'=>0, 'earned' => 0, 
                                            'early'=>0,
                                            'wthd' => 1, 'note'=>$note] + $extra); 
                }
            ...
            // NOT HOLD Type
                ...
                if($perc > 0 && $amount > 0){
                    ...
                    // add to yearly details
                    array_push($yearly,  [  'type' => $type, 'type_name' => $type_name,
                                            'year' => $not_hold_year, 'terms'=>$terms,  'hold'=>0,
                                            'rate' => $rate,'perc'=>$perc,  'no_fund'=>0,
                                            'amount'=>$amount, 'principal'=> $amount, 
                                            'total'=> $final, 'earned' => $ne, 
                                            'wthd' => 1, 'note'=>$note] + $extra); 

                }else{
                    array_push($yearly,  [  'type' => $type, 'type_name' => $type_name,
                                            'year'=>$not_hold_year, 'terms'=>$terms, 'hold'=>0, 
                                            'rate' => $rate,'perc'=>0, 'no_fund'=>1,
                                            'amount'=> 0, 'principal'=>0, 
                                            'total'=>0, 'earned' => 0, 
                                            'wthd' => 1, 'note'=>$note] + $extra); 
                }
            ...
            // log_info("$type_name -> early withdrawl ".($early_wth ? 'YES' : 'NO'), $yearly);
    ...
    public function compare($model_id){
        ...
        $compare['inv_strategies'] = $this->inv_strategies; if($compare['inv_strategies']['ffbp'])unset($compare['inv_strategies']['ffbp']);
