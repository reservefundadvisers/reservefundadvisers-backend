...
{  
    ...
    private function set_config($data){
        ...
        return set_property($configTable, json_encode($data));
    }
    ...
    public function simulation($data){
        ...
        // deficit data
        $deficits  = get_elements($deficitTable, ['model_id'=>$data['model_id'], 'user_id'=>$auth->uid()], "*", "ORDER BY year"); // get deficits
        ...
        foreach($deficits as $deficit){
            ...
            if(is_valid($deficit_array[$deficit['year']], 'monthly_fees')){
                ...
                for($i=intval($deficit['year']); $i<$period; $i++){
                    ...
                    $auto_monthly_fees_inc[$i] = $i == 0 ? ($auto_monthly_fees[$i] - $monthly_fees) / $monthly_fees : 
                                                           ($auto_monthly_fees[$i] - $auto_monthly_fees[$i - 1]) / $auto_monthly_fees[$i - 1] ;
                }
            }
        ...
        for($i=0; $i<$period; $i++){
            ...
            // auto monthly fees
            $is_auto_mf = check_val($erase_deficit, 'monthly_fees_auto', false);
            ...
            */
            ...
            $this->calculateYearData(   $starting_amount,                                         
                                        ...
                                        $spending, 
                                        $erase_deficit,
                                        $ltim_perc, $auto_monthly_fees_ltim_amount[$i], $ltim_withdrawn, $ltim_yoc, $ltim_str, $is_ltim_enabled,
                                        $year_calculations, $i );
            ...
            if($is_ltim_enabled){
                ...
                if(!isset($erase_deficit['ltim_wth']) && $rule_ltim_cover == 1 && $year_calculations['fa'] < 0){
                    ...
                    $deficit_to_cover = abs($year_calculations['fa']);  // abs() year deficit
            ...
            if(!$is_lowering_fees && !$applying_cushion /* && ($auto_monthly_fees_stop_year == -1 || $i < $auto_monthly_fees_stop_year)  */
                && $is_auto_mf /* $rule_mf_auto */ && $year_calculations['fa'] < 0 /* && array_has($deficit_years, $i) */ && !array_has($processed_deficits, $i) ){
                
                // log_info("Deficit Year ".$i.": ".$year_calculations['fa']);


                $this->updateMonthlyFeeInc( $i, ($year_calculations['fa']/(12*$housing)), $auto_monthly_fees, $auto_monthly_fees_inc, 
                                            $rule_mf_perc, $monthly_fees, $rule_cushion_fund, /* $auto_monthly_fees_stop_year */);
    ...
    public function calculateYearData(  $starting_amount,                                         
            ...
            // ad LTIM withdrawn amout to starting amount
            ...
            // expenses
            $total_expenses = $impl_ltim ? -1 * $ltim_yoc : $loss_purchase + $spending;

            log_info($total_expenses);
    ...
    private function calculateLTIM($spendings, $startAt = 0){
        ...
        if(empty($this->ltim_strategy))return;

        // log_info($this->ltim_strategy);


        $buckets = []; $deficit_sum = 0;
        $from = check_val($this->ltim_strategy, 'start_year', 0) + $startAt;
    ...
    private function get_closest_inc_year($year, $start_year = 0, &$auto_monthly_fees, &$auto_monthly_fees_inc, $max_inc, $base_mf, $deficit){

        // if first year skip
        if($year == 0)return ['year'=>$year, 'start_year'=>0, 'total_years'=>1, 'inc'=>$this->estimateMonthlyFeeInc(0, $auto_monthly_fees[0], $deficit, $max_inc)];
        ...
        $inc = $this->estimateMonthlyFeeInc($total_years, $base_mf, $deficit, $max_inc);
    ...
    private function updateMonthlyFeeInc(   $year, $deficit, 
                                            ...
                                            $default_monthly_fee, $cushion_fund = 0 /* , $stop_at_year = -1 */){
        
        

            // get closest year with % to increase by and total years to increase
            $inc_infos = $this->get_closest_inc_year($year, 0, $auto_monthly_fees, $auto_monthly_fees_inc, $max_inc, '', $deficit);
    ...
    private function get_rules($model_id){
        ...
        if(empty($model_id) || !belongs_to_client($modelsTable, $model_id) )
            // return ["ltim_enabled"=>0, "ltim_perc"=>1,"ltim_cover"=>1, "ltim_lower"=>0, "ltim_yoc"=>3, "mf_perc"=>0, "mf_auto"=>0, "cushion_fund"=>0,"inv_keep"=>1];
            return ["ltim_enabled"=>1, "ltim_perc"=>0,"ltim_cover"=>0, "ltim_lower"=>0, "ltim_yoc"=>3, "mf_perc"=>0, "mf_auto"=>0, "cushion_fund"=>0,"inv_keep"=>1];
        ...
        // return !empty($rules) ? $rules : ["ltim_enabled"=>0, "ltim_perc"=>1,"ltim_cover"=>1, "ltim_yoc"=>3, "ltim_lower"=>0,"mf_perc"=>$max_mf_inc, "mf_auto"=>0, "cushion_fund"=>$cushion_fund, "inv_keep"=>1];
        return !empty($rules) ? $rules : ["ltim_enabled"=>1, "ltim_perc"=>0,"ltim_cover"=>0, "ltim_yoc"=>3, "ltim_lower"=>0,"mf_perc"=>$max_mf_inc, "mf_auto"=>0, "cushion_fund"=>$cushion_fund, "inv_keep"=>1];
    }
