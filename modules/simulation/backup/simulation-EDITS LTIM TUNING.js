...
var Simulation = function(){
    ...
    Simulation.prototype.eraseDeficit = function(at, callback){
        ...
        var showBuckets = function(ltim_str, startYear, ltim_p, ltim_ne, ltim_acc_p, ltim_acc_ne){
            ...
            view_modal(modal, 'LTIM Buckets ' + startYear, null, true, 'xl', false);
        }
        ...
        context.timelineDeficit(null, function(year, isMF = false){
        ...
        var eraseDeficit_init = function(){
            ...
            var ltim_amount = !objEmpty(deficit_data) && deficit_data.ltim_amount != undefined ? parseInt(deficit_data.ltim_amount) : 0;
            var ltim_perc = !objEmpty(deficit_data) && deficit_data.ltim_perc != undefined ? parseFloat(deficit_data.ltim_perc) : (original_data.ltim_spl > 0 /* && general_rules['ltim_perc']*/ ? original_data.ltim_perc /* general_rules['ltim_perc'] */ :  0) ;
            var ltim_withdrawed = !objEmpty(deficit_data) && deficit_data.ltim_wth != undefined ? parseFloat(deficit_data.ltim_wth) : 0;
            var ltim_reinv = !objEmpty(deficit_data) && deficit_data.ltim_reinv != undefined ? parseFloat(deficit_data.ltim_reinv) : (original_data.ltim_reinv != undefined ? original_data.ltim_reinv : 1);
            ...
            deficit_box.find('.simulation-rule').each(function(){
                ...
                if(item.is(':checkbox')){
                    ...
                    item.val(is_perc ? parseFloat(value * 100): value).change(function(){
                        var item = $(this)
                            key = item.data('key'),
                            is_perc = item.hasClass('percent'), 
            ...
            ltim_ne_label.html(formatMoney(original_data.ltim_ne, 0));
                ...
                ltim_perc_reset.unbind().click(function(){ 
                                                    ...
                                                    apply_changes();
                                                }).toggleClass('invisible', deficit_data['ltim_perc'] == undefined);
                ...
                ltim_perc_apply.unbind().click(function(){           
                                                ...
                                                if(ltim_perc > value && has_deficits(at, 'ltim_perc')){
                                                    ...
                                                    update_deficit_data('ltim_perc', value, (value < 0 || value > 1));
                                                    // update_deficit_data('ltim_amount_chk', ltim_amount_chk.is(':checked') ? 1 : 0, value == 0);
            ...
            var ltim_acc_label = deficit_box.find('.ltim-acc'),    
                ...
                ltim_reinv_switch = deficit_box.find('.ltim-reinv'),
                ...
                ltim_reinv_switch.unbind().change(function(){
                                                            var checked = $(this).is(':checked');
                                                            update_deficit_data('ltim_reinv', checked ? '1' : '0');
                                                            apply_changes();
                                            }).prop('checked', ltim_reinv == 1);
            ...
            var inv_strategy_update_allocations = function(){
                
                inv_strategy_inputs.filter('[data-type="perc"]').each(function(){
                    var index = $(this).data('index'),
                        perc = parseFloat($(this).val()) / 100;
                        
                    deficit_box.find('.inv-strategy-p-a[data-index="'+index+'"]').html(formatMoney(original_data.ip * perc, 0))
            ...
            setTimeout(function(){ context.timelineDeficit() }, 200);
    ...
    Simulation.prototype.timelineDeficit = function(all_data, cb, initOnly = false){
        ...
        var mf_timeline_handle_year_arrows = function(){
            var mf_timeline = box.find('.simulation-timeline-mf-container');
    ...
    Simulation.prototype.init = function(model_id, callbacks){ 
            ...
            this.compare(model_id, function(data){
                ...
                var print_table = function(){
                    ...
                    var chart_series = {"remaining_amount":[], "compound":[], "total_expenses":[], "loan":[], "loan_payments":[], "collections":[], "ltim":[], "ltim_i":[], "ltim_yoc":[], "deficits":[]};
                    var chart_xaxis = [];
                    //chart_element.html('');
                    ...
                    if(context.simulation_data.model.simulation_rules.ltim_enabled == 1){
                        chart_initial_data.push({ name: 'Years Of Projected Spendings', type: 'line', barMaxWidth:15, 
                                                    itemStyle: {color: '#2732b0'}, emphasis: { focus: 'series' }, data: chart_series.ltim_yoc });
    ...
    Simulation.prototype.deficit = function(model_id, year, deficit_data, cb, splits, unsplit){
                                    ...
                                    if(unsplit)params['unsplit']=unsplit;
                                     ...
                                     console.log(deficit_data, params);
