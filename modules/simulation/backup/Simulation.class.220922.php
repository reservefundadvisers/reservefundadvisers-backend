...
{  
    ...
    public function simulation($data){
        ...
        $last_monthly_fees = $default_monthly_fees;

        // used by monthly_fees_year deficit data
        $deficit_monthly_fees_inc = array_fill(0, $period, 0);
        ...
        $rule_mf_keep = check_val($model['simulation_rules'], 'mf_keep', 0);
        ...
        foreach($deficits as $deficit){
            ...
            // add monthly fees increase 
            $monthly_fees_inc_from = check_val($deficit_array[$deficit['year']], 'monthly_fees_year_from', 0) - 1;
            $monthly_fees_inc_by = check_val($deficit_array[$deficit['year']], 'monthly_fees_year', 0);
            
            if($monthly_fees_inc_from >= 0 && $monthly_fees_inc_from < $period && $monthly_fees_inc_by > 0){
                // for($i=$monthly_fees_inc_from; $i <= $deficit['year']; $i++){    // upto the selected year
                for($i=$monthly_fees_inc_from; $i <=  $period; $i++){  // throughout the whole period
                    $deficit_monthly_fees_inc[$i] += $monthly_fees_inc_by;
                }
            }
        ...
        for($i=0; $i<$period; $i++){
            ...
            if(isset($erase_deficit['monthly_fees'])){
                ...
                $last_monthly_fees = $monthly_fees;
            }else if(!$rule_mf_keep){
            ...
            $this->calculateYearData(   $starting_amount,                                         
                                        $monthly_fees + $deficit_monthly_fees_inc[$i], $housing,
                                        $invest_strategy,
                                        $year_calculations["lp"],
                                        $loan_amount,
                                        $loan_payment,
    ...
    private function get_rules($model_id){
        ...
        if(empty($model_id) || !belongs_to_client($modelsTable, $model_id) )
            return ["ltim_enabled"=>0, "ltim_perc"=>1,"ltim_cover"=>1,"mf_keep"=>1,"inv_keep"=>1];

        $rules = parse_json(check_val(get_element($simRulesTable, ['model_id'=>$model_id, 'user_id'=>$auth->uid()]), 'rules', []), []);

        return !empty($rules) ? $rules : ["ltim_enabled"=>0, "ltim_perc"=>1,"ltim_cover"=>1,"mf_keep"=>1,"inv_keep"=>1];
    }
