...
class Models
{
    ...
    public function process($cmd, $data)
    {
        ...
        switch ($cmd) {
            ...
            case 'edit_actual':
                $response = $this->edit_actual($data);
                break;
            case 'save_actual':
                $response = $this->save_actual($data);
                break;
            case 'reset_actual':
                $response = $this->reset_actual($data);
                break;
        }
    ...
    public function edit($data)
    {
        ...
        $element['model'] = get_element_join(
            $modelsTable,
            ['id' => $data['model_id']],
        ...
        if (check_val($data, 'force', 0) == 0) {
            ...
            if (
                exists($simDeficitTable, ['model_id' => $data['model_id']]) ||
